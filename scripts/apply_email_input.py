...
def main() -> None:
    ...
    run_block = "  async function runScan(value){var raw=String(value||'').trim();if(!raw)return;currentInputType=looksLikeEmail(raw)?'email':'url';currentUrl=currentInputType==='email'?emailValue(raw):normalize(raw);if(!currentUrl)return;input.value=currentUrl;loading();busy(true);track('analyze');try{var endpoint=currentInputType==='email'?'/api/email-check':'/api/check';var payload=currentInputType==='email'?{input:currentUrl}:{url:currentUrl};var r=await fetch(endpoint,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});var d=await r.json();renderQuick(d);if(currentInputType==='url')trackScanSummary(d)}catch(e){renderQuick({inputType:currentInputType,error:currentInputType==='email'?'The email address check could not complete.':'The safety check could not complete.',safety:{status:'unknown',riskScore:0,signals:[]}})}finally{busy(false)}}"
