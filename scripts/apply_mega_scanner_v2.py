...
(function(){
  ...
  var baseFetch=window.fetch.bind(window);window.fetch=async function(){var args=arguments,res=await baseFetch.apply(window,args);try{var target=typeof args[0]==='string'?args[0]:(args[0]&&args[0].url)||'';if(String(target).indexOf('/api/analyze')>=0){res.clone().json().then(function(d){renderMega(d);document.dispatchEvent(new CustomEvent('cist:mega-result',{detail:d}))}).catch(function(){})}}catch(e){}return res};
