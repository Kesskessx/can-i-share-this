...
(function(){
  ...
  async function createShare(summary,parentResultId){
    var response=await fetch('/api/share-result',{
      method:'POST',
      headers:{'content-type':'application/json'},
      body:JSON.stringify({summary:summary,parentResultId:parentResultId||null})
    });
  ...
  function labelButton(){
    var button=document.getElementById('cist-mega-share');
    if(button&&!button.disabled&&button.textContent!=='Share on X')button.textContent='Share on X';
  }
  ...
  document.addEventListener('click',async function(event){
    var button=event.target&&event.target.closest?event.target.closest('#cist-mega-share'):null;
    if(!button)return;
    event.preventDefault();
    event.stopPropagation();
    if(event.stopImmediatePropagation)event.stopImmediatePropagation();

    var result=window.cistMegaLastResult;
    if(!result||!result.shareSummary||button.disabled)return;

    var popup=window.open('about:blank','_blank');
    if(popup)try{popup.opener=null}catch(_){}
    button.disabled=true;
    button.textContent='Preparing X post…';

    try{
      var shared=await createShare(result.shareSummary,null);
      track('share_x',shared.id,null);
      var target=intent(shared.url,result.shareSummary);
      if(popup)popup.location.replace(target);
      else window.open(target,'_blank','noopener,noreferrer');
      button.textContent='Shared on X';
    }catch(error){
      if(popup)popup.close();
      button.textContent='Share on X';
    }finally{
      setTimeout(function(){button.disabled=false;button.textContent='Share on X'},1400);
    }
  },true);
})();
</script>
