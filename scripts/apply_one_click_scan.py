...
(function(){
  ...
  var actions=document.getElementById('actions'),deep=document.getElementById('deep'),deepConfirm=document.getElementById('deep-confirm'),reputation=document.getElementById('reputation');
  var advice=document.getElementById('advice'),adviceText=document.getElementById('advice-text'),whyVerdict=document.getElementById('why-verdict'),whyList=document.getElementById('why-list'),contextualPrevention=document.getElementById('contextual-prevention'),riskMeter=document.getElementById('risk-meter');
  if(!form||!input||!paste||!analyze||!card||!actions||!deepConfirm||!reputation)return;

  var deepRunning=false,quickStatus='unknown',quickClass='',quickWhy='';
  ...
  function resetButton(){if(!deepRunning&&!analyze.disabled)analyze.textContent=buttonLabel()}
  ...
  function setWhy(items){if(!whyVerdict||!whyList)return;whyList.innerHTML=items.map(function(x){return '<li>'+x+'</li>'}).join('');whyVerdict.classList.remove('hidden')}
  function startFullCheck(){
    if(deepRunning||looksLikeEmail(input.value)||reputation.classList.contains('reputation-alert'))return;
    deepRunning=true;quickStatus=localStatus();quickClass=card.className;quickWhy=whyList?whyList.innerHTML:'';
  ...
  function finishFullCheck(){
    ...
    var unavailable=text.indexOf('unavailable')>=0||text.indexOf('could not')>=0;
    ...
    if(noKnown&&quickStatus==='high'){
      ...
      if(quickWhy)setWhy([quickWhy.replace(/^<li>|<\/li>$/g,'')]);
      return;
    }
    ...
    if(unavailable||true){
      if(quickStatus==='high'){
        ...
        adviceText.textContent='Do not open this link. Verify the sender or use the official website yourself.';
      }else{
        ...
        adviceText.textContent='If you do not recognize this link, do not open it. Try again later or verify the sender another way.';
      }
      advice.classList.remove('hidden');
    }
  }
  ...
  paste.addEventListener('click',async function(e){
    e.preventDefault();e.stopImmediatePropagation();paste.disabled=true;var old=paste.textContent;paste.textContent='Pasting…';
