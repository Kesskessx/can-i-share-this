...
@media(min-width:1100px){
  ...
  #cist-scam-signals-rail .cist-signal-head{
    ...
    padding:13px 14px!important;border-bottom:1px solid var(--line)!important;
  }
  #cist-scam-signals-rail .cist-signal-title{
    color:var(--muted)!important;font-size:10px!important;font-weight:900!important;letter-spacing:.09em!important;text-transform:uppercase!important;
  }
  #cist-scam-signals-rail .cist-signal-live{
    display:inline-flex!important;align-items:center!important;gap:6px!important;color:var(--muted)!important;font-size:9px!important;font-weight:800!important;white-space:nowrap!important;
  }
  #cist-scam-signals-rail .cist-signal-live:before{
    ...
    background:var(--amber)!important;box-shadow:0 0 0 4px color-mix(in srgb,var(--amber) 10%,transparent)!important;
  }
  #cist-scam-signals-rail .cist-signal-primary{padding:17px 14px 15px!important}
  #cist-scam-signals-rail .cist-signal-number{
    display:block!important;color:var(--text)!important;font-size:clamp(30px,3vw,38px)!important;font-weight:920!important;
  ...
  #cist-scam-signals-rail .cist-signal-primary-label{
    display:block!important;margin-top:5px!important;color:var(--muted)!important;font-size:10px!important;font-weight:750!important;
  }
  #cist-scam-signals-rail .cist-signal-list{display:grid!important;border-top:1px solid var(--line)!important}
  #cist-scam-signals-rail .cist-signal-row{
    ...
    padding:11px 14px!important;border-bottom:1px solid color-mix(in srgb,var(--line) 78%,transparent)!important;
  }
  #cist-scam-signals-rail .cist-signal-row:last-child{border-bottom:0!important}
  #cist-scam-signals-rail .cist-signal-row span{min-width:0!important;color:var(--muted)!important;font-size:9px!important;line-height:1.3!important}
  #cist-scam-signals-rail .cist-signal-row strong{
    color:var(--text)!important;font-size:11px!important;font-weight:900!important;text-align:right!important;font-variant-numeric:tabular-nums!important;
  }
  #cist-scam-signals-rail .cist-signal-row.cist-signal-hot strong{color:var(--amber)!important}
  #cist-scam-signals-rail .cist-signal-foot{
    padding:10px 12px!important;border-top:1px solid var(--line)!important;color:var(--muted)!important;font-size:8px!important;line-height:1.4!important;text-align:center!important;
  }
}
...
def main():
    ...
    required=['Scam signals','Suspicious redirects','Phishing indicators','Lookalike domains','Risky downloads','metricsOnly:true','min-width:1100px','15000']
    for token in required:
        if token not in source:
            raise RuntimeError(f'Scam signals rail guard failed: missing {token}')
    HOME.write_text(source,encoding='utf-8')
    print('Applied desktop-only live scam signals rail')
