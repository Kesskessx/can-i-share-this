...
(function(){
  ...
  function explanationReasons(d){var out=[],items=Array.isArray(d.explanation&&d.explanation.reasons)?d.explanation.reasons:[];items.forEach(function(r){if(out.length>=3)return;var title=clean(r&&r.title),detail=clean(r&&r.detail);if(title||detail)out.push({title:title||'Safety signal',detail:detail,kind:clean(r&&r.severity).toLowerCase()==='positive'?'ok':'warning'})});return out}
  ...
  function render(d){
    ...
    var domainHtml=conflict?'<div class="cist-explain-v3-box cist-explain-v3-domain"><span class="cist-explain-v3-label">Official domain expected</span><strong>'+esc(cap(conflict.brand))+' appears to be claimed</strong><span>Detected: '+esc(conflict.seen||'unknown domain')+'</span>'+(conflict.official?'<span class="cist-explain-v3-domain-code">'+esc(conflict.official)+'</span>':'<span>Use the official company website from a trusted source.</span>')+'</div>':'';
