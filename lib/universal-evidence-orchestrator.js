...
const { detectBrandClaims, normalizeBrand, normalizeHost, brandDomainStatus }=require('./brand-registry');
...
function normalizedExistingElements(base){
  ...
  return{
    urls:[...(e.urls||[]),...(a.urls||[]),...(m.urls||[])],domains:[...(e.domains||[])],emails:[...(e.emails||[]),...(a.emails||[]),...(m.emails||[])],phones:[...(e.phones||[]),...(a.phones||[]),...(m.phones||[])],crypto:[...(e.crypto||[])],qr:[...(e.qr||[]),...(a.qr_values||[])],files:[...(e.files||[])],socialProfiles:[...(e.socialProfiles||[])],claimedBrands:[...(e.claimedBrands||[]),...(a.claimed_brands||[]),...(m.claimedBrands||[])]
  };
}
...
async function runCandidate(c,context){
  if(c.type==='url')return capture(checkHandler,{url:c.value});
  if(c.type==='email')return capture(emailHandler,{input:c.value});
  if(c.type==='crypto')return capture(cryptoHandler,{input:c.value});
  if(c.type==='social-profile')return capture(socialProfileHandler,{input:c.value});
  if(c.type==='phone')return{status:200,body:analyzePhone(c.value,context)};
...
function brandRelations(evidence){const out=[];for(const brand of evidence.claimedBrands)for(const domain of evidence.domains.slice(0,6)){if(SOCIAL_HOSTS.has(domain))continue;const r=brandDomainStatus(brand,domain);if(r.status==='confirmed')out.push({type:'claimed-brand-domain',status:'confirmed',positive:true,brand:r.brand,websiteDomain:domain,detail:`${domain} is in the recognized official-domain set for ${r.brand}.`});else if(r.status==='conflict')out.push({type:'claimed-brand-domain',status:'conflict',positive:false,severity:'high',brand:r.brand,websiteDomain:domain,detail:`The content claims ${r.brand}, but ${domain} is not a recognized official domain for that identity.`})}return unique(out,x=>`${x.type}:${x.brand}:${x.websiteDomain}`,12)}
...
async function relationshipChecks(evidence,checks,base){
  const sites=websiteUrls(evidence),tasks=[];
  for(const social of evidence.socialProfiles.slice(0,2))tasks.push(verifyWebsiteSocialRelationship({websites:sites,socialProfile:social}));
  const socialResults=(await Promise.all(tasks)).flat();
  return unique([...socialResults,...emailWebsiteRelations(evidence),...brandRelations(evidence),...emlRelations(base),...profileWebsiteRelations(checks,evidence)],x=>`${x.type}:${x.detail}`,24);
}
...
function buildRelationGraph(evidence,relations){const nodes=[],edges=[],map=new Map();const add=(type,value,extra={})=>{value=clean(value,400);if(!value)return null;const key=`${type}:${value.toLowerCase()}`;if(map.has(key))return map.get(key);const id=`r${nodes.length+1}`;nodes.push({id,type,value,...extra});map.set(key,id);return id};for(const b of evidence.claimedBrands)add('brand',b);for(const d of evidence.domains)add('domain',d);for(const e of evidence.emails)add('email',`[mailbox]@${emailDomain(e)}`);for(const s of evidence.socialProfiles)add('social',`${s.platform} @${s.username}`);for(const p of evidence.phones)add('phone','Phone number');for(const c of evidence.crypto)add('crypto','Crypto address');for(const f of evidence.files)add('file',f);for(const rel of relations||[]){let a=null,b=null;if(rel.type==='website-social'){a=add('domain',rel.websiteDomain);b=add('social',`${String(rel.socialPlatform).toUpperCase()} @${rel.socialHandle}`)}else if(rel.type==='website-email'){a=add('domain',rel.websiteDomain);b=add('email',`[mailbox]@${rel.emailDomain}`)}else if(rel.type==='claimed-brand-domain'){a=add('brand',rel.brand);b=add('domain',rel.websiteDomain)}else if(rel.type==='email-from-replyto'){a=add('email-domain','From');b=add('email-domain','Reply-To')}else if(rel.type==='profile-bio-website'){a=add('social','Social profile');b=add('domain',rel.websiteDomain)}if(a&&b)edges.push({from:a,to:b,relation:rel.type,status:rel.status,strength:rel.status==='confirmed'?'strong':rel.status==='conflict'?'conflict':'unknown'})}return{version:'4.0',nodes:nodes.slice(0,40),edges:edges.slice(0,60)}}

function additionalCorrelations(evidence,domains,relations,base){const out=[];const domainBy=new Map((domains||[]).map(x=>[x.domain,x]));for(const rel of relations||[]){if(rel.type==='claimed-brand-domain'&&rel.status==='conflict'){const d=domainBy.get(rel.websiteDomain);if(d&&Number.isFinite(d.rdap&&d.rdap.ageDays)&&d.rdap.ageDays<=30)out.push({type:'brand-new-domain',severity:'high',title:'Brand mismatch combined with a recently registered domain',detail:`${rel.websiteDomain} conflicts with the claimed ${rel.brand} identity and was registered about ${d.rdap.ageDays} days ago.`})}}
...
function aggregateRisk(base,checks,domains,pages,relations,external,correlations){const values=[riskFromBody(base),...checks.map(x=>x.risk),...pages.map(x=>x.status)];for(const d of domains){if((d.signals||[]).some(s=>s.code==='domain-brand-conflict'))values.push('high');else if((d.signals||[]).some(s=>s.code==='domain-very-new'||s.code==='domain-new'))values.push('caution')}for(const r of relations)if(r.status==='conflict')values.push(r.severity==='high'?'high':'caution');for(const x of external.urlChecks||[])if(x.dangerous)values.push('high');for(const c of correlations)values.push(c.severity==='high'?'high':'caution');return strongest(values)}

async function orchestrateUniversalEvidence({detectedType,originalInput='',baseResult={},externalConsent=false}){
  const evidence=extractEvidence({detectedType,originalInput,baseResult});const candidates=candidateList(evidence,detectedType,originalInput),existing=Array.isArray(baseResult.crossChecks)?baseResult.crossChecks:[];const existingKeys=new Set(existing.map(x=>`${clean(x.type,50)}:${clean(x.value,1200).toLowerCase()}`));const missing=candidates.filter(x=>!existingKeys.has(`${x.type}:${x.type==='phone'?'phone number':x.type==='crypto'?'[crypto address]':x.type==='email'?x.value.replace(/^[^@]+@/,'[mailbox]@').toLowerCase():x.value.toLowerCase()}`));
  const sites=websiteUrls(evidence),domainTargets=unique(evidence.domains.filter(d=>!SOCIAL_HOSTS.has(d)),x=>x,MAX_DOMAINS);
  const [scanSettled,domainSettled,pageSettled,external]=await Promise.all([
    Promise.allSettled(missing.map(x=>runCandidate(x,evidence.visible))),
    Promise.allSettled(domainTargets.map(d=>analyzeDomain(d,{claimedBrands:evidence.claimedBrands}))),
    Promise.allSettled(sites.slice(0,MAX_WEBPAGES).map(analyzeWebpage)),
    externalChecks(evidence,externalConsent)
  ]);
  ...
  const relations=await relationshipChecks(evidence,checks,baseResult);
  const correlations=additionalCorrelations(evidence,domains,relations,baseResult);
  const finalRisk=aggregateRisk(baseResult,checks,domains,pages,relations,external,correlations);
