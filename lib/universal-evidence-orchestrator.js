...
function extractEvidence({detectedType,originalInput,baseResult}){
  ...
  const phoneText=crypto.reduce((text,address)=>text.split(address).join(' '),visible);
  const phones=unique([...existing.phones,...extractPhones(phoneText)].filter(value=>{const digits=String(value).replace(/\D/g,'');return digits.length>=8&&digits.length<=15&&!crypto.some(address=>String(address).includes(String(value).trim()));}),x=>String(x).replace(/\D/g,''),10);
