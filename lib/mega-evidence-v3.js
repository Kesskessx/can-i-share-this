...
function positiveEvidence(result) {
  const out = [];
  ...
  if (socialLow) out.push({ type: 'social-check-low', strength: 'supporting', title: 'No strong impersonation pattern in the profile check', detail: clean(socialLow.summary, 360) || 'The specialized social-profile check did not return a caution or high-risk result.' });
  const urlLow = checks.find(x => x && x.type === 'url' && risk(x.risk) === 'low');
  if (urlLow) out.push({ type: 'url-check-low', strength: 'supporting', title: 'No major URL warning in the linked website check', detail: clean(urlLow.summary, 360) || 'The specialized URL check did not return a caution or high-risk result.' });
  return uniq(out, x => `${x.type}:${x.websiteDomain || ''}:${x.handle || ''}`, 6);
}
function identityConsistency(result, positives) {
  ...
  if (hasSocial && hasWebsite) return { level: unavailable ? 'medium' : 'medium', score: unavailable ? 0.5 : 0.56, label: 'Identity partly consistent', detail: unavailable ? 'The screenshot connects a profile and website, but the website relationship could not be independently checked.' : 'A profile and website were detected, but a reciprocal link to the exact profile was not confirmed from the website homepage.' };
...
function augmentGraph(result, positives) {
  const graph = result.evidenceGraph && typeof result.evidenceGraph === 'object' ? result.evidenceGraph : { version: '3.0', nodes: [], edges: [] };
  ...
  return { version: '3.0', nodes: nodes.slice(0, 30), edges: edges.slice(0, 40) };
}
function recalcFinalRisk(result) {
  ...
  const mismatches = result.brandMismatch && Array.isArray(result.brandMismatch.items) ? result.brandMismatch.items : [];
  const mismatchRisk = mismatches.some(x => x && x.severity === 'high') ? 'high' : mismatches.length ? 'caution' : 'low';
  return maxRisk([analysisRisk, mismatchRisk, ...checks, ...correlations]);
}
...
function upgradeMegaResult(result) {
  ...
  let reasons = dedupeReasons((baseEx.reasons || []).filter(r => !weakAccountText(`${r && r.title || ''} ${r && r.detail || ''}`)));
  if (!reasons.length && positives.length) reasons = positives.slice(0, 2).map(x => ({ severity: 'positive', title: x.title, detail: x.detail }));
  ...
  return {
    ...
    megaScanner: {
      ...
      correlationMode: 'multi-element + relationship'
    }
  };
}
...
module.exports = { enrichScanResult, upgradeMegaResult, dedupeReasons, identityConsistency, recalcFinalRisk };
