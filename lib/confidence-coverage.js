...
function applyConfidenceCoverage(input){
  ...
  for(const c of arr(input.crossChecks)){
    ...
    if(Number(c&&c.status||200)<400&&clean(c&&c.risk,20)!=='unknown')addCompleted(`check:${type}:${completed.length}`,`${type.replace(/-/g,' ')} specialized check`,`${type} scanner`,1.5);
  ...
  if(eligible>0){
    if(reputation.consent===true){
      const checked=arr(reputation.urlChecks).filter(x=>x&&x.checked!==false).length+arr(reputation.cryptoChecks).filter(x=>x&&x.checked).length;
  ...
  const coverage={
    completed:completed.length,relevant,ratio:Number(ratio.toFixed(2)),
