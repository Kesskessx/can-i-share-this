...
function safeBase64(value){
  ...
  if(!text||text.length%4===1||!/^[A-Za-z0-9+/]*={0,2}$/.test(text)||(text.includes('=')&&text.length%4!==0))return null;
  try{const b=Buffer.from(text,'base64');return b.length&&b.toString('base64').replace(/=+$/,'')===text.replace(/=+$/,'')?b:null}catch(_){return null}
}
function parseFilePayload(file){
  ...
  if(encoded.length>4*Math.ceil(MAX_BYTES/3))throw new Error('File too large. Maximum size: 3 MB.');
  const bytes=safeBase64(encoded);
