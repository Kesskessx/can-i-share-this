...
function assessEmail({ local, domain, hadUnicodeDomain = false, dnsInfo, rdapInfo = {} }) {
  ...
  if (dnsInfo.dmarcKnown && !dnsInfo.hasDmarc) {
    ...
    if (Number.isFinite(dmarc.pct) && dmarc.pct < 100) {
      addSignal(signals, 'email-dmarc-partial', 'low', 'DMARC policy applies to only part of mail', `The DMARC pct value is ${dmarc.pct}%, so the published handling policy may not be requested for every message.`, 2);
    }
  }
...
async function analyzeEmailAddress(input) {
  ...
  return {
    ...
    email: {
      ...
      dmarcPct: dnsInfo.dmarc ? dnsInfo.dmarc.pct : null,
